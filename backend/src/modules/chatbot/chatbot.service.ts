...
export class ChatbotService {
  ...
  private async buildLiveData(user: any) {
    ...
    if (user.role === "admin") {
      const [
        ...
        this.prisma.achievement.groupBy({ by: ["academicYear"], _count: { _all: true } }),
        this.prisma.achievement.findMany({
          orderBy: { createdAt: "desc" },
          take: 5,
          include: { student: { select: { fullName: true, studentId: true, department: true } } },
        }),
      ]);
      ...
      return {
        ...
        departmentBreakdown: departmentStats.map((row) => ({ _id: row.department, students: row._count._all })),
        achievementCategories: categoryStats.map((row) => ({ _id: row.category, achievements: row._count._all })),
        achievementYears: yearStats.map((row) => ({ _id: row.academicYear, achievements: row._count._all })),
    ...
    if (user.role === "student") {
      student = await this.prisma.student.findUnique({ where: { userId: user.id } });
    } else if (user.role === "parent") {
      const parentProfile = await this.prisma.parentProfile.findUnique({ where: { userId: user.id } });
      if (parentProfile?.studentDbId) {
        student = await this.prisma.student.findUnique({ where: { id: parentProfile.studentDbId } });
      }
    }
  ...
  async chatWithBot(user: any, body: any) {
    ...
    if (!geminiKey) {
      return { reply: "Chatbot is not configured. Add GEMINI_API_KEY in the backend .env." };
    }
    ...
    const listModels = async () =>
      fetch(`https://generativelanguage.googleapis.com/${apiVersion}/models?key=${geminiKey}`);

    const pickSupportedModel = (models: any[] = []) =>
      models.find((m: any) => (m.supportedGenerationMethods || []).includes("generateContent"))?.name?.replace("models/", "");
